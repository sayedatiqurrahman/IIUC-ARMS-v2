...
export default function ReadmeEditor({ folder, isOwner }: ReadmeEditorProps) {
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch('/api/github/readme', {
        ...
        body: JSON.stringify({ folder, content: editContent }),
      });
      const data = await res.json();
      if (data.success) {
      ...
      {editing ? (
        <div className="p-3">
          <p className="text-[0.68rem] text-dark-text3 mb-2">
            Add links using markdown format: <code className="bg-dark-bg3 px-1 rounded">- [Title](https://url)</code>
          </p>
          <textarea
        ...
        <div className="p-3">
          {links.length === 0 ? (
            <p className="text-[0.72rem] text-dark-text3 text-center py-2">No links shared yet.</p>
          ) : (
