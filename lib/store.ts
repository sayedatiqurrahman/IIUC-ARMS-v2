...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  getSemesterCourses: (semId, departmentId?) => {
    ...
    // Detect README.md and .md files per course
    uploadTree.forEach((item: any) => {
      if (item.type !== 'blob') return;
      const fileName = item.path.split('/').pop();
  ...
  getCourseCategories: (semId, courseCode, departmentId?, midFinal?: string | null) => {
    ...
    // Collect README.md and .md file folder paths for this course
    ...
    if (!midFinal) {
      ...
      // Check README.md at Mid/Final level (prefix match: any README under mid/ or final/)
